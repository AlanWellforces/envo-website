...
// Single regional purchase CTA for series pages: exactly ONE distributor at
// a time, chosen by the visitor's region (useRegion always resolves a region —
// nz-ap default, geo-updated). The label stays neutral "Find local
// distributor" — never "Buy from X". A borderless dropdown lets the visitor
// switch region; the CTA link + fine print follow.

import { useEffect, useRef, useState } from 'react'
import { PURCHASE_CHANNELS } from '@/data/purchase-channels'
import { useRegion } from '@/components/region/RegionProvider'
import { REGION_TO_DISTRIBUTOR, DISTRIBUTORS, type SeriesPurchaseLinks } from '@/data/distributors'
...
export function FindDistributorCta({
  ...
  // "New Zealand · Asia-Pacific · Wellforces" — region + serving distributor
  const label = (r: (typeof PURCHASE_CHANNELS)[number]) =>
    `${r.regionLabel} · ${DISTRIBUTORS[REGION_TO_DISTRIBUTOR[r.id]].name}`
  const channel = PURCHASE_CHANNELS.find((c) => c.id === region)

  return (
    <>
      {/* buttons-first: equal-width CTA + datasheet, region + fine print below */}
      ...
      <div className="wtb-block" aria-live="polite">
        <div className="wtb-region">
          ...
          <div className={`wtb-dd${picking ? ' open' : ''}`} ref={ddRef}>
            ...
            >
              <b>{channel ? label(channel) : distributor.name}</b>
